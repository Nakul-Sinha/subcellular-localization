...
(p≈0.3 to 0.5)** so the model cannot memorize family→location shortcuts (every test family
...
in does literally nothing. Final inference = **5-fold probability-bagging × 4 to 8-view D4
...
| Image intensity | dark/sparse: protein ch ~68 % nonzero, nucleus ~42 %, per-image max ≈ 120 to 160 | **Naive ImageNet normalization crushes the signal** → per-channel norm + percentile contrast stretch. |
...
| Family groups | **1,186** families; sizes 2 to 374 (median 3); **test families all unseen** | Leave-family-out CV is mandatory; in-distribution within-family label agreement ≈ **0.877** → text/family is a powerful *in-distribution* shortcut that must be regularized away. |
...
raw macro-F1 of ~0.45 to 0.58 maps to LocSkill ≈ **0.42 to 0.56**.
...
the classes with prevalence 0.02 to 0.10. The whole pipeline is engineered around lifting
...
- **Why not ViT/Swin:** a recent leave-family-out HPA study (arXiv 2505.22926) shows
  **Swin-B and ViT-B/16 catastrophically overfit** (val 0.42 to 0.65 → test 0.018 to 0.18),
...
- **`max_len = 256`** (covers the ~166-word test tail at ~1.3 to 1.5 biomedical tokens/word).
  512 doubles cost for no gain; 128 truncates the family/process tail.
- **Treat text as a weak prior:** freeze for the first 1 to 2 epochs, then fine-tune at a
...
### 3.3 Fusion
- **Late fusion**: `concat(img_emb, text_emb) → 2-layer MLP (dropout 0.3 to 0.5) → 16
...
- **Text modality-dropout p = 0.3 to 0.5**: zero the entire text vector for a fraction of
...
- **Per-channel normalization** using dataset mean/std (compute once on train), **plus
  per-image 1 to 99.5 percentile contrast stretch** (or `log1p`) before normalizing. This is
  one of the cheapest, highest-ROI steps: naive ImageNet stats map mean-pixel-3 to 16 images
...
- **Primary contender, Asymmetric Loss (ASL,** Ben-Baruch/Ridnik ICCV'21**)**:
  `gamma_neg=4, gamma_pos=0 to 1, clip=0.05`. Targets the ~1.93-positive / ~14-negative
...
| Component | Setting |
|---|---|
| Optimizer | AdamW; image LR **2e-4**, text LR **1e-5**; weight decay 1e-4 to 5e-2 |
| Schedule | Cosine decay + 3 % warmup; **8 to 15 epochs**; early-stop on **OOF macro-F1** |
| Regularization | drop-path 0.1 to 0.2; MLP/head dropout 0.3 to 0.5; **EMA (decay 0.999)** → predict with EMA weights; optional SWA over last ~20 % epochs |
...
| Mixup/CutMix (multi-label-correct) | **union-of-labels** for CutMix; **ratio-mixed soft targets** (Mixup-BCE) for Mixup; mild α 0.2 to 0.4. *Never average one-hot targets.* |
| Batch / resolution | ~48 to 64 @ 320 px on A10G (fits 24 GB) |
...
- **Several smaller diverse models beat one big model, the single most consistent winning
  finding across HPA 2019 & 2021** (adversarially verified, 3-0; every top team, 4 to 18 models).
...
- **D4 TTA:** average sigmoids over 4 to 8 flip/rotation views.
...
- **Targets:** raw macro-F1 ≈ **0.45 to 0.58** → **LocSkill ≈ 0.42 to 0.56** (description's stated
  band; constant baseline ≈ 0; `F1_base ≈ 0.043`). Rare/secondary classes are the ceiling, 
  HPA winners plateaued at AP ≈ 0.31 to 0.34 on weak classes even with 4 channels + external
  data.
- **MEASURED (this solution, H100 leave-family-out OOF, StratifiedGroupKFold on `family_group_id`):**
  ...
  Run 3's **rawF1 0.457 is inside the stated 0.45 to 0.58 band**. The **threshold-as-logit-bias
  ...
  5-fold + multi-seed bagging, loss A/B/C (ASL/DB), Small backbone, heavier aug, each ~+0.02 to 0.05
...
- **Key risks:**
  ...
  - *No external HPAv18 data* (banned/unavailable), drove the biggest rare-class gain for
    ...
    thresholds.
  - *Runtime*, measure 5 to 10-model bag × TTA on real A10G; cap TTA/ensemble to fit 30 min.
...
- **Ensemble several smaller diverse models (4 to 18), trained with different losses &
...
- **BCE base loss + focal/class-weights + rare-class oversampling + (optionally) a Lovász
  term, with per-class threshold tuning** is the proven imbalance recipe *(3-0)*. → §4 to 5.
